...
function route_query(?string $key = null, mixed $default = null): mixed
{
    ...
    return route_query_internal_array_get($query, $key, $default);
}
...
function route_query_only(string ...$keys): array
{
    ...
    foreach ($keys as $key) {
        if (harbor_is_blank($key) || ! route_query_internal_array_has($query, $key)) {
            continue;
        }

        $only[$key] = route_query_internal_array_get($query, $key);
    }
...
function route_query_arr(string $key, array $default = []): array
{
    ...
    if (is_string($value)) {
        $decoded = route_query_internal_decode_json($value, true);
...
function route_query_obj(string $key, ?object $default = null): ?object
{
    ...
    if (is_string($value)) {
        $decoded = route_query_internal_decode_json($value, false);
...
{
    return route_query_internal_array_has(route_queries(), $key);
}

/** Private */
function route_query_internal_decode_json(string $value, bool $assoc): mixed
{
...
function route_query_internal_array_get(array $array, string $key, mixed $default = null): mixed
{
...
function route_query_internal_array_has(array $array, string $key): bool
{
    if (array_key_exists($key, $array)) {
        return true;
    }
