...
{
    $web_user = session_get(auth_web_session_key(), null);

    return auth_normalize_user($web_user);
}
...
function auth_web_login(array $user): bool
{
    ...
    return session_set(auth_web_session_key(), $normalized_user);
}

function auth_web_logout(): bool
{
    return session_forget(auth_web_session_key());
}

/** Private */
function auth_web_session_key(): string
{
    $web_session_key = auth_value_to_string(config_get('auth.web.session_key', 'auth_web_user'));
