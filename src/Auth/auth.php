...
{
    $attempt_resolver = auth_attempt_resolver();
...
/** Private */
function auth_attempt_resolver(): ?\Closure
{
    $web_attempt_resolver = auth_value_to_callable(config_get('auth.web.attempt_resolver'));
    if ($web_attempt_resolver instanceof \Closure) {
        return $web_attempt_resolver;
    }

    return auth_value_to_callable(config_get('auth.api.attempt_resolver'));
}

function auth_normalize_user(mixed $user): ?array
{
...
function auth_value_to_string(mixed $value): ?string
{
...
function auth_value_to_int(mixed $value, int $default = 0): int
{
    if (is_int($value)) {
        return $value;
    }
...
function auth_value_to_callable(mixed $value): ?\Closure
{
